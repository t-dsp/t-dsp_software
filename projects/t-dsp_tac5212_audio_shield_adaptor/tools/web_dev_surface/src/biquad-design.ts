...
// Q27 conversion. Clamped to int32 range. Mirrors the inline functions in
// the firmware's TAC5212_Biquad.h.
function toQ27(coef: number): number {
  const scale = 1 << 27;
  const scaled = coef * scale;
  if (scaled > 2147483520) return 0x7FFFFFFF;
  if (scaled < -2147483648) return -0x80000000;
  // Round toward zero like the firmware's static_cast<int32_t>; this is
  // what JS does with `| 0` for in-range values, but for safety we use
  // Math.trunc + clamp.
  return Math.trunc(scaled) | 0;
}

function fromQ27(reg: number): number {
  const invScale = 1 / (1 << 27);
  // Treat reg as signed 32-bit even if JS sees it as a regular number.
  const signed = reg | 0;
