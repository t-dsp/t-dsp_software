// Supersaw slot panel — config UI for slot 6 (JP-8000-style 5-osc lead).
//
// New panel built for the slot-architecture rebuild. Layout follows
// mpe-slot-panel.ts: ON/OFF header, slot housekeeping (volume + MIDI
// channel), then engine knobs (detune, mix, filter, ADSR, portamento)
// and the chorus-depth shim. A compact preset grid sits between the
// header and the parameter rows so the standard JP-8000 starting points
// (Trance Lead, Hoover, Anthem, etc.) are one click away.
//
...
//
// The legacy supersaw-panel.ts (also preset-grid-based) is left in
// place untouched per the parallel-agent contract. Once the user
// confirms this replacement is what they want, the legacy file can
// be deleted.
...
export function supersawSlotPanel(state: MixerState, dispatcher: Dispatcher): HTMLElement {
  const root = document.createElement('div');
  // Reuse the .mpe-panel selector for shared styling (preset grid,
  // section titles, segmented controls). The supersaw-specific bits
  // hang off .supersaw-panel for any future overrides.
  root.className = 'mpe-panel supersaw-panel';
...
function buildOnRow(state: MixerState, dispatcher: Dispatcher): HTMLElement {
  ...
  state.supersaw.on.subscribe((on) => {
    ...
    btn.title = on ? 'Click to mute Supersaw' : 'Click to un-mute Supersaw';
  });
...
function buildPresetCard(preset: Preset, state: MixerState, dispatcher: Dispatcher): HTMLElement {
  ...
  state.supersaw.activePresetId.subscribe((id) => {
...
function buildControls(state: MixerState, dispatcher: Dispatcher): HTMLElement {
  ...
  section.appendChild(buildSliderRow(
    'Volume', state.supersaw.volume,
    0, 1, (v) => v.toFixed(2),
    (v) => dispatcher.setSupersawVolume(v),
  ));
  section.appendChild(buildMidiChannelRow(state, dispatcher));
  section.appendChild(buildSliderRow(
    'Detune', state.supersaw.detuneCents,
    0, 100, (v) => `${v.toFixed(1)} ¢`,
    (v) => dispatcher.setSupersawDetune(v),
  ));
  section.appendChild(buildSliderRow(
    'Mix', state.supersaw.mixCenter,
    0, 1, (v) => v < 0.1 ? 'Sides' : v > 0.9 ? 'Center' : v.toFixed(2),
    (v) => dispatcher.setSupersawMixCenter(v),
  ));
  section.appendChild(buildSliderRow(
    'Cutoff', state.supersaw.cutoffHz,
    50, 20000, (v) => `${Math.round(v)} Hz`,
    (v) => dispatcher.setSupersawCutoff(v),
    { log: true },
  ));
  section.appendChild(buildSliderRow(
    'Resonance', state.supersaw.resonance,
    0.707, 5.0, (v) => v.toFixed(2),
    (v) => dispatcher.setSupersawResonance(v),
  ));
  section.appendChild(buildSliderRow(
    'Attack', state.supersaw.attack,
    0.001, 3.0, (v) => v < 1 ? `${Math.round(v * 1000)}ms` : `${v.toFixed(2)}s`,
    (v) => dispatcher.setSupersawAttack(v),
    { log: true },
  ));
  section.appendChild(buildSliderRow(
    'Decay', state.supersaw.decay,
    0.001, 3.0, (v) => v < 1 ? `${Math.round(v * 1000)}ms` : `${v.toFixed(2)}s`,
    (v) => dispatcher.setSupersawDecay(v),
    { log: true },
  ));
  section.appendChild(buildSliderRow(
    'Sustain', state.supersaw.sustain,
    0, 1, (v) => v.toFixed(2),
    (v) => dispatcher.setSupersawSustain(v),
  ));
  section.appendChild(buildSliderRow(
    'Release', state.supersaw.release,
    0.001, 5.0, (v) => v < 1 ? `${Math.round(v * 1000)}ms` : `${v.toFixed(2)}s`,
    (v) => dispatcher.setSupersawRelease(v),
    { log: true },
  ));
  section.appendChild(buildSliderRow(
    'Portamento', state.supersaw.portamentoMs,
    0, 1000, (v) => v < 1 ? 'Off' : `${Math.round(v)} ms`,
    (v) => dispatcher.setSupersawPortamento(v),
  ));
  section.appendChild(buildSliderRow(
    'Chorus', state.supersaw.chorusDepth,
    0, 1, (v) => v.toFixed(2),
...
function buildMidiChannelRow(state: MixerState, dispatcher: Dispatcher): HTMLElement {
  const row = document.createElement('div');
  row.className = 'mpe-row';
  ...
  state.supersaw.midiChannel.subscribe((c) => {
    btns.forEach((b, i) => b.classList.toggle('active', i === c));
  });
