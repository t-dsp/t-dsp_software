...
// =============================================================================
// DAC dynamics: Distortion Limiter (page 25) and DRC (page 28)
// =============================================================================
//
// Both subsystems are programmable-coefficient blocks with 8 × 32-bit values
// each. Coefficients are MSB-first, two's complement. The chip's POR defaults
// (per datasheet §8.2.10 and §8.2.13) are pre-tuned and immediately usable;
// the driver provides preset structs that mirror them.
//
// Enable bits live on page 1:
//   - Distortion limiter:  P1_R45 bit 7 (MISC_CFG0.EN_DISTORTION)
//   - DRC per-channel:     P1_R36 bits 3..0 (AGC_DRC_CFG.DRC_CHx_EN)
//   - Limiter routing:     P1_R35 (LIMITER_CFG, INP_SEL[7:6] + OUT_SEL[5:4])

namespace dac_limiter {
    constexpr uint8_t PAGE = 25;
    constexpr uint8_t COEF_SIZE = 4;
    constexpr uint8_t COEF_COUNT = 8;

    constexpr uint8_t OFFSET_ATTACK_COEFF   = 0x60;
    constexpr uint8_t OFFSET_RELEASE_COEFF  = 0x64;
    constexpr uint8_t OFFSET_ENV_DECAY      = 0x68;
    constexpr uint8_t OFFSET_THRESHOLD_MAX  = 0x6C;
    constexpr uint8_t OFFSET_THRESHOLD_MIN  = 0x70;
    constexpr uint8_t OFFSET_INFLECTION_PT  = 0x74;
    constexpr uint8_t OFFSET_SLOPE          = 0x78;
    constexpr uint8_t OFFSET_RESET_COUNTER  = 0x7C;
}
...
namespace dac_drc {
    ...
    constexpr uint8_t OFFSET_MAX_GAIN           = 0x1C;
    constexpr uint8_t OFFSET_MIN_GAIN           = 0x20;
    constexpr uint8_t OFFSET_ATTACK_TC          = 0x24;
    constexpr uint8_t OFFSET_RELEASE_TC         = 0x28;
    constexpr uint8_t OFFSET_RELEASE_HOLD_COUNT = 0x2C;
    constexpr uint8_t OFFSET_RELEASE_HYST       = 0x30;
    constexpr uint8_t OFFSET_INV_RATIO          = 0x34;
    constexpr uint8_t OFFSET_INFLECTION_PT      = 0x38;
}

// Page 1 enable / routing registers for DRC + limiter.
namespace dac_dynamics_ctrl {
    constexpr uint8_t PAGE = 1;
    constexpr uint8_t LIMITER_CFG = 0x23;  // INP_SEL[7:6], OUT_SEL[5:4]
    constexpr uint8_t AGC_DRC_CFG = 0x24;  // DRC_CH1..4 enable in bits 3..0
    constexpr uint8_t MISC_CFG0   = 0x2D;  // EN_DISTORTION bit 7, DSP_AVDD_SEL bit 1

    constexpr uint8_t MASK_EN_DISTORTION = 0x80;
    // MISC_CFG0 bit 1: 0 = "Reserved" (POR), 1 = SAR AVDD data fed to DSP.
    // Required HIGH for DSP Limiter, BOP, and DRC blocks to operate on valid
    // data — leaving it at POR causes squeal/instability when those blocks
    // are enabled. Set once at codec init.
    constexpr uint8_t MASK_DSP_AVDD_SEL  = 0x02;
    constexpr uint8_t MASK_DRC_CH1_EN    = 0x08;
    constexpr uint8_t MASK_DRC_CH2_EN    = 0x04;
    constexpr uint8_t MASK_DRC_CH3_EN    = 0x02;
    constexpr uint8_t MASK_DRC_CH4_EN    = 0x01;
    constexpr uint8_t MASK_DRC_ALL_CH    = 0x0F;

    // LIMITER_CFG INP_SEL[7:6]:
    //   00 = max(ch0, ch1) — most defensive, default
    //   01 = ch1 only
    //   10 = ch0 only
    //   11 = avg(ch0, ch1)
    constexpr uint8_t LIMITER_INP_SEL_MAX = 0x00;
    constexpr uint8_t LIMITER_INP_SEL_AVG = 0xC0;

    // LIMITER_CFG OUT_SEL[5:4]:
    //   00 = applied to both channels (default)
    //   01 = ch1 only
    //   10 = ch0 only
    //   11 = applied to neither (effectively bypassed)
    constexpr uint8_t LIMITER_OUT_SEL_BOTH = 0x00;
    constexpr uint8_t LIMITER_OUT_SEL_NONE = 0x30;
}
